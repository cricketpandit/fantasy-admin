...
import ViewContest from './ViewCricketSeasonContest';



const CricketSeasonContest = (props) => {
    ...
    const pageData = async (page = activepage) => {
        ...
        path = apiUrl.get_cricket_season_constest + `?${queryString}`;
        getData(path)
    };

    const updateLeaguePlayers = async (series_id) => {
        let path = `/crons/update-cricket-league-player/?series_id=${series_id}`;
    ...
    const handleSearching = async () => {
        ...
        setQuery(queries)
        path = apiUrl.get_cricket_season_constest + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&min_entry_fee=' + `${min_entry_fee}` + '&max_entry_fee=' + `${max_entry_fee}` + '&min_winning_amount=' + `${min_winning_amount}` + '&max_winning_amount=' + `${max_winning_amount}` + '&contest_size=' + `${contest_size}` + '&contestType=' + `${contestType}` + '&categoryType=' + `${category_id}` + '&start_date=' + `${startdate == '' || startdate == null ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' || enddate == null ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
        getData(path)
    };

    const resetSearch = async () => {
        let path = apiUrl.get_cricket_season_constest + '?page=1&itemsPerPage=10';
        getData(path)
    }
                ...
                <Col>
                    <Card>
                        <CardHeader className="align-items-center d-flex">
                            Cricket Season Contests
                            <div className="ml-auto">
                                <Link to="/cricket/add-season-contest" className="btn btn-primary">
                                    <i className="fa fa-plus mr-1"></i>Add Cricket Season Contest
                                </Link>
                            </div>
                        </CardHeader>
                        <CardBody>
                                    ...
                                    <Col lg={12}>
                                        <Form>
                                            <Row>
                                                ...
                                                <Col md={2} sm={6}>
                                                    <FormGroup className="">
                                                        <select name={'category'} className={"form-control"} value={category_id} onChange={(e) => { setQuizCategory(e.target.value); }} >
                                                            ...
                                                            <option key={'fun'} value={'fun'}>{'Fun'}</option>
                                                        </select>
                                                    </FormGroup>
                                                </Col>
                                                <Col md={2} sm={6}>
                                                ...
                                                <Col md={2} sm={6}>
                                                    <FormGroup className="">
                                                        <Input type="text" placeholder="Min Winning Amount" value={min_winning_amount} className="form-control"
                                                            onChange={(e) => { setMinWinningAmount(e.target.value) }} />
                                                    </FormGroup>
                                                </Col>
                                                <Col md={2} sm={6}>
                                                    <FormGroup className="">
                                                        <Input type="text" placeholder="Max Winning Amount" value={max_winning_amount} className="form-control"
                                                            onChange={(e) => { setMaxWinningAmount(e.target.value) }} />
                                                    </FormGroup>
                                                </Col>
                                                <Col md={2} sm={6}>
                                                    <FormGroup className="">
                                                        <Input type="text" placeholder="Min Entry Fee" value={min_entry_fee} className="form-control"
                                                            onChange={(e) => { setMinEntryFee(e.target.value) }} />
                                                    </FormGroup>
                                                </Col>
                                                <Col md={2} sm={6}>
                                                    <FormGroup className="">
                                                        <Input type="text" placeholder="Max Entry Fee" value={max_entry_fee} className="form-control"
                                                    ...
                                                    <td className="text-center w-40">
                                                        {
                                                            _.isEmpty(props.match.params.id) ?
                                                                ...
                                                                <div>
                                                                    <Link to={{ pathname: `/cricket/edit-season-contests/${item.id}` }} className="btn-link">
                                                        ...
                                                        <button className="btn circle_btn btn-sm mr-1" onClick={(e) => updateLeaguePlayers(item?.series_id)} type="button" title="Update Players"> Update Players</button>
...
export default CricketSeasonContest;
