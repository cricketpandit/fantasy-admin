...
const QuizCategoryList = (props) => {
  ...
  const pageData = async (page = activepage) => {
    ...
    path = apiUrl.getQuizCategory + `?${queryString}`;
    getData(path)
  };
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)
    path = apiUrl.getQuizCategory + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&start_date=' + `${startdate == '' || startdate == null ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' || enddate == null ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path)
  };

  const resetSearch = async () => {
    let path = apiUrl.getQuizCategory + '?page=1&itemsPerPage=10';
    getData(path)
  }
          ...
          <Card>
            <CardHeader className="align-items-center d-flex">
              ...
              <div className="ml-auto">
                <Link to="/questions/category/add" className="btn btn-primary">
                ...
                <Table hover bordered responsive className="mt-3 text-center">
                  ...
                  <tbody>
                    {users.map((item, key) => {
                      return (
                        <tr key={key}>
                          <td className="text-left">{Helper.ternaryOperator(item?.type === 'liveEvent','Live Event',item?.type === 'perception' ? "Perception": item?.type)}</td>
                          ...
                          <td className="text-center w-40">
                            {
                              _.isEmpty(props.match.params.id) ?
                                <div>
                                  ...
                                  <Link to={{ pathname: `/questions/category/edit/${item?._id}` }} className="btn-link">
