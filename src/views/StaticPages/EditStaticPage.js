...
const EditStaticPage = (props) => {
    ...
    const onSumbit = async e => {
        setIsubmit(true);
        let postJson = {
            title: title, slug: slug, content: draftToHtml(convertToRaw(content.getCurrentContent()))
        };
       
        let path = apiUrl.updateStaticPages + '/' + `${id}`;
        const fr = await Helper.put(token,postJson, path);
        const res = await fr.response.json();
        if (fr.status === 200) {
            if (res.success) {
                setIsubmit(false);
                props.history.push('/static-pages');
                Toast.fire({
                    type: "success",
                    title: res.msg,
                })
    ...
    return (
        <React.Fragment>
            <ValidatorForm onSubmit={onSumbit}>
                <Card>
                    <CardHeader>
                        <CardTitle><h4>Edit Static Pages</h4></CardTitle>
                    </CardHeader>
                    <CardBody>
                        <Row>
                            <Col md={12}>
                                <FormGroup>
                                    <Label>Title</Label>
                                    <TextValidator readOnly type="text" name="title" className="form-control bg-transparent" placeholder="Page Title"
