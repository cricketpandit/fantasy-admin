...
const TestFeilding = (props) => {
    ...
    const onSubmit = async data => {
        ...
        let postJson = {
            ...
            testIndirectRunOut: data.testIndirectRunOut.trim()

        };

        let path = apiUrl.update_test_feilding;
    ...
    const getData = async () => {
        let path = apiUrl.get_test_feilding;
                            ...
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <input type="number" max={11} onChange={handleCatch} min={0} step="0.1" name="fieldingCatch" placeholder="Catch" autoComplete="off"
                                        className="form-control col-md-8" defaultValue={testData.fieldingCatch} ref={register({ required: 'Required' })} />
                                </FormGroup>
                            </Col>
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <input type="number" min={0} step="0.1" name="fieldingStumpRunOut" placeholder="Stumping Direct" autoComplete="off"
                                        className="form-control col-md-8" defaultValue={testData.fieldingStumpRunOut} ref={register({ required: 'Required' })} />
                                </FormGroup>
                            </Col>
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <input type="number" min={0} step="0.1" name="fieldingRunOutThrower" placeholder="Run-out" autoComplete="off"
                                        className="form-control col-md-8" defaultValue={testData.fieldingRunOutThrower} ref={register({ required: 'Required' })} />
                                </FormGroup>
                            </Col>
                            <Col md={6}>
                                <FormGroup>
                                    <Label className={'col-md-4 pull-left mt-2'}>Run-out( Not Direct hit ):</Label>
                                    <input type="number" min={0} step="0.1" name="testIndirectRunOut" placeholder="Run-out ( Not Direct hit )" autoComplete="off"
                                        className="form-control col-md-8" defaultValue={testData?.testIndirectRunOut} ref={register({ required: 'Required' })} />
                                </FormGroup>
                            </Col>
                        </Row>
                    </CardBody>
