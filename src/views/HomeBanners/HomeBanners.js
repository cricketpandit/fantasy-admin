...
const HomeBanners = (props) => {
  ...
  const pageData = async (page = activepage) => {
    ...
    path = apiUrl.get_home_banners + `?${queryString}`;
    getData(path)
  };
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)
    path = apiUrl.get_home_banners + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&start_date=' + `${startdate == null || startdate == "" ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == null || enddate == "" ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path)
  };

  const resetSearch = async () => {
    let path = apiUrl.get_home_banners + '?page=1&itemsPerPage=10';
    getData(path)
  }
  ...
  const isUserOrAdmin = user_type === "admin" || (user_type === "editor" && module.delete === 1);
  const isUserOrAdminEdit = user_type === "admin" || (user_type === "editor" && module.edit === 1);
          ...
          <Card>
            <CardHeader className="align-items-center d-flex">
              ...
              <div className="ml-auto">
                <Link to="/add-home-banner" className="btn btn-primary">
                  <i className="fa fa-plus mr-1"></i>Add Home Banner
                </Link>
              </div>
            </CardHeader>
            <CardBody>
                      ...
                      return (
                        <tr key={key}>
                          ...
                          <td className="text-align-center">
                            {(item.media_type == 'image') &&  ((item.image !=='') ? <img alt="Banner" src={bannerPath + item.image} height="100" width="200" /> : <img alt="Banner" src={bannerPath + 'avtar.png'} height="100" width="200" />)}
                            {(item.media_type == 'video') && <video height={80} width={200} controls >
                              <source src={bannerPath + item.image} type="video/mp4" />
                            </video>}
                          </td>
                          <td className="text-align-center">
                          ...
                          <td className="text-align-center w-40">
                            {
                              _.isEmpty(props.match.params.id) ?
                                <div>
                                  ...
                                  {isUserOrAdmin && <Delete item={item} refreshData={pageData} />}
                                  {isUserOrAdminEdit &&                                   
                                  <Link to={{ pathname: `/edit-home-banner/${item.id}` }} className="btn-link">
